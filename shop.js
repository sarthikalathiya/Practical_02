...
const SHOP_SCHEDULE = [
  ...
  function formatOpeningTime(hours) {
    if (hours < 24) {
      return `Shop is Currently Closed. and it will be open after ${roundToDecimal(hours)} Hrs`;
    } else {
      const days = Math.floor(hours / 24);
      const remainingHours = roundToDecimal(hours % 24);
      const dayText = days === 1 ? 'Day' : 'Days';
      return `Shop is Currently Closed. and it will be open after ${days} ${dayText} and ${remainingHours} Hrs`;
    }
  }
  ...
  function getShopStatus() {
    ...
    if (isShopOpen()) {
      ...
      const hoursUntilOpening = getNextOpeningTime();
      return formatOpeningTime(hoursUntilOpening);
    }
  }
